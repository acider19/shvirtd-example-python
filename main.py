...
def index(request: Request, ip_address: Optional[str] = Depends(get_client_ip)):
    ...
    if final_ip is None:
        ip_display = "похоже, что вы направляете запрос в неверный порт(например curl http://127.0.0.1:5000). Правильное выполнение задания - отправить запрос в порт 80."
    else:
        ip_display = final_ip
